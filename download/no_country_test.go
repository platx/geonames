...
func Test_Client_NoCountry(t *testing.T) {
	t.Parallel()

	caller := func(client *Client, ctx context.Context) ([]GeoName, error) {
		res := make([]GeoName, 0)

		err := client.NoCountry(ctx, func(parsed GeoName) error {
	...
	testCases := []testSuite[GeoName]{
		...
		},
		{
			...
			exp: exp[GeoName]{
				res: []GeoName{},
				err: errors.New("parse file \"no-country.txt\" in archive => parse file => context canceled"),
			},
		},
		{
			...
			exp: exp[GeoName]{
				res: []GeoName{},
				err: errors.New("parse file \"no-country.txt\" in archive => file not found in archive"),
			},
		},
		{
			...
			exp: exp[GeoName]{
				res: []GeoName{},
				err: errors.New("parse file \"no-country.txt\" in archive => open zip archive => zip: not a valid zip file"),
			},
		},
		{
			name: "no content",
