...
func Test_Client_ByCountry(t *testing.T) {
	...
	testCases := []testSuite[GeoName]{
				...
				httpClient: testutil.MockHTTPClient(func(m *testutil.HTTPClientMock) {
					m.On(
						...
						&http.Response{
							...
							Body:       testutil.MustOpen(testdata.FS, "byCountrySuccess.zip"),
						},
						nil,
					)
				}),
		...
		},
		{
			...
			exp: exp[GeoName]{
				res: []GeoName{},
				err: errors.New("parse file \"US.txt\" in archive => parse file => context canceled"),
			},
		},
		{
			...
			args: args{
				httpClient: testutil.MockHTTPClient(func(m *testutil.HTTPClientMock) {
					m.On("Do", mock.Anything).Return(
						&http.Response{
							StatusCode: http.StatusOK,
							Body:       testutil.MustOpen(testdata.FS, "byCountryMissing.zip"),
						},
						nil,
					)
				}),
			...
			exp: exp[GeoName]{
				res: []GeoName{},
				err: errors.New("parse file \"US.txt\" in archive => file not found in archive"),
			},
		},
		{
			...
			args: args{
				httpClient: testutil.MockHTTPClient(func(m *testutil.HTTPClientMock) {
					m.On("Do", mock.Anything).Return(
						&http.Response{
							StatusCode: http.StatusOK,
							Body:       testutil.MustOpen(testdata.FS, "byCountryInvalid.zip"),
						},
						nil,
					)
				}),
			...
			exp: exp[GeoName]{
				res: []GeoName{},
				err: errors.New("parse file \"US.txt\" in archive => open zip archive => zip: not a valid zip file"),
			},
		},
		{
			name: "no content",
